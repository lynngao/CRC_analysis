...
auprc <- function(dataset, ds) {
  ...
  pr.curve(scores.class0 = dataset[,"CRC"], weights.class0 = dataset$status, curve = T)$auc.integral
}


AUPRC_LODO = as.data.frame(matrix(NA,nrow = 2, ncol = 6))
colnames(AUPRC_LODO) = c("yu","hannigan", "feng", "vogtmann","zeller", "thomas")
...
yu_known$status = test$status
feng_known$status = test$status
vogtmann_known$status = test$status
zeller_known$status = test$status
thomas_known$status = test$status
yu_unknown$status = test$status
feng_unknown$status = test$status
vogtmann_unknown$status = test$status
zeller_unknown$status = test$status
thomas_unknown$status = test$status
...
rownames(pred) = rownames(test)
colnames(pred) = c("CRC", "status")
pred$CRC = ((yu_AUC[,1]-0.5)*yu_known[,3] + (feng_AUC[,1]-0.5)*feng_known[,3] + (vogtmann_AUC[,1]-0.5)*vogtmann_known[,3] + (zeller_AUC[,1]-0.5)*zeller_known[,3] + (thomas_AUC[,1]-0.5)*thomas_known[,3] + (yu_AUC[,2]-0.5)*yu_unknown[,3] + (feng_AUC[,2]-0.5)*feng_unknown[,3] + (vogtmann_AUC[,2]-0.5)*vogtmann_unknown[,3] + (zeller_AUC[,2]-0.5)*zeller_unknown[,3] + (thomas_AUC[,2]-0.5)*thomas_unknown[,3]) / (yu_AUC[,1] + feng_AUC[,1] + vogtmann_AUC[,1] + zeller_AUC[,1] + thomas_AUC[,1] + yu_AUC[,2] + feng_AUC[,2] + vogtmann_AUC[,2] + zeller_AUC[,2] + thomas_AUC[,2] - 5) 

pred$status = test$status
AUPRC_LODO[2,2] = pr.curve(scores.class0 = pred[pred$status == "CRC", 1], scores.class1 = pred[pred$status == "control", 1], curve = T)$auc.integral #0.6642674
...
pred$status = test$status
AUPRC_LODO[2,4] = pr.curve(scores.class0 = pred[pred$status == "CRC", 1], scores.class1 = pred[pred$status == "control", 1], curve = T)$auc.integral #0.8379878
...
pred$status = test$status
AUPRC_LODO[2,5] = pr.curve(scores.class0 = pred[pred$status == "CRC", 1], scores.class1 = pred[pred$status == "control", 1], curve = T)$auc.integral #0.8250475
...
pred$status = test$status
AUPRC_LODO[2,6] = pr.curve(scores.class0 = pred[pred$status == "CRC", 1], scores.class1 = pred[pred$status == "control", 1], curve = T)$auc.integral #0.8193283

AUPRC_LODO = round(AUPRC_LODO, digits = 2)
write.csv(AUPRC_LODO, "/Users/lynngao/Desktop/results/AUPRC/AUPRC_LODO1.csv")
