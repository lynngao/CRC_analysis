...
cross_model_stacking <- function(ds, prob_known, prob_unknown, save_LOSO_prob=FALSE){
	...
	if (save_LOSO_prob == TRUE){
		write.csv(prob_df, "/directory/LOSO_prob_ds.csv") #save this for calculating AUPRC
	}
...
LODO_model_stacking <- function(test, prob_known1, prob_unknown1, prob_known2, prob_unknown2, 
	prob_known3, prob_unknown3,prob_known4, prob_unknown4, prob_known5, prob_unknown5, return_pred=FALSE) {
