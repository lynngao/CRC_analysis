...
within_rf <- function(dataset, num_trees, num_rep, df){
	#renormalization
	...
	for (i in 1:num_rep){
		sample_crc = sample.int(n=nrow(dataset[dataset$status == "CRC",]),size=floor(0.8*nrow(dataset[dataset$status == "CRC",])),replace=F)
  		...
  		df[i,1] <- auc(test$status, pred[,1])
  	}

  	return(df)
}

within_lasso <- function(dataset, num_rep, df, lambda = "lambda.min"){
	#renormalization
	dataset[,1:(ncol(dataset)-1)] = dataset[,1:(ncol(dataset)-1)]/rowSums(dataset[,1:(ncol(dataset)-1)]) 
	...
	for (i in 1:num_rep){
		sample_crc = sample.int(n=nrow(dataset[dataset$status == "CRC",]),size=floor(0.8*nrow(dataset[dataset$status == "CRC",])),replace=F)
  		...
  		training = rbind(training_crc,training_control)
  		test = dataset[rownames(dataset)%!in%rownames(training),]
        training$status = as.factor(ifelse(training$status == "CRC", 1, 0))
  		test$status = as.factor(ifelse(test$status == "CRC", 1, 0))
  
  		lassofit <- glmnet::cv.glmnet(x = as.matrix(training[,-ncol(training)]), y = training$status, family = "binomial")

  		pred <- as.vector(predict(lassofit, newx=as.matrix(test[,-ncol(test)]), s=lambda,type="response"))

  		df[i,1] <- auc(test$status, pred)
  	}

  	return(df)
}

within_svm <- function(dataset, num_rep, df){
	#renormalization
	dataset[,1:(ncol(dataset)-1)] = dataset[,1:(ncol(dataset)-1)]/rowSums(dataset[,1:(ncol(dataset)-1)]) 
	#log10 transformation
	dataset[,1:(ncol(dataset)-1)] = log10(dataset[,1:(ncol(dataset)-1)]+1e-100)
	...
	for (i in 1:num_rep){
		sample_crc = sample.int(n=nrow(dataset[dataset$status == "CRC",]),size=floor(0.8*nrow(dataset[dataset$status == "CRC",])),replace=F)
  		...
  		rf <- train(
    		status ~ ., data = known_training, method = "svmLinear", metric = "ROC",
    		trControl = trainControl(method = "repeatedcv", number = 10, search = "grid", summaryFunction = twoClassSummary, classProbs = TRUE, savePredictions = TRUE)
  		)
  		pred <- predict(rf, test, type="prob")
  		df[i,1] <- auc(test$status, pred[,1])
...
cross_lasso <- function(training, training_status, test, test_status, num_rep, df, lambda="lambda.min", col=1){
	training = training/rowSums(training) #renormalize
	#log10 transformation
...
cross_svm <- function(training, training_status, test, test_status, num_rep, df, col=1){
	...
	for (i in 1:30){
		svm <- train(status ~ ., data = training, method = "svmLinear", metric = "ROC",
	    trControl = trainControl(method = "repeatedcv", number = 10, search = "grid", summaryFunction = twoClassSummary, classProbs = TRUE, savePredictions = TRUE))
